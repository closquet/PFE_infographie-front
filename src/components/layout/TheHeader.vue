<template>
    <header class="site-header" :style="headerStyle">
        ...
        <section aria-labelledby="page-title">
            ...
            <section aria-labelledby="page-page-title">
                <h2 v-if="$route.meta.displayedSubName"
                    aria-level="2"
                    class="site-header__title site-header__title--resource"
                    id="page-resource-title"
                    role="heading">
                ...
                <div v-if="$route.name === 'account'" class="site-header__user">
                    ...
                    <input type="file" ref="avatarInput" @change="processFile($event)" style="display: none;">
                    <div :class="['site-header__user__avatar-container', {'site-header__user__avatar-container--no-avatar': !avatar}]">
                        <img class="site-header__user__avatar" v-if="!form.selectedFile && avatar" :src="avatar" alt="Avatar">
                        <div :key="$route.name" class="site-header__user__new-avatar" v-if="form.selectedFile && avatar" :style="`background-image: url('${avatar}')`"></div>
                        <button title="Ajouter ma photo" v-if="!avatar" @click="selectAvatar">
                ...
                <button v-if="form.selectedFile" class="site-header__saveAvatarBtn" @click.prevent="saveAvatar">
                    Enregistrer le nouvel avatar
                </button>

                <button v-if="form.selectedFile" class="site-header__cancelAvatarBtn" @click.prevent="cancelSelectedAvatar">
                    Annuler
                </button>

                <ul class="avatar-response response-errors" v-if="responseErrors.length && $route.name === 'account'">
                    <li class="response-errors__error" v-for="(error, index) in responseErrors" :key="index">
                        {{error}}
                    </li>
                </ul>
                <span v-if="responseSuccess && $route.name === 'account'" class="avatar-response response-success">
...
<script>
    ...
    import BackOfficeMenu from "@/views/admin/subViews/parts/BackOfficeMenu";
    ...
    export default {
        ...
        methods: {
            ...
            cancelSelectedAvatar() {
                if (this.user.data.avatar)
                    this.avatar = `${this.axios.defaults.baseURL}/storage/${this.user.data.avatar}`;
                else
                    this.avatar = null;
            ...
            processFile(event) {
                if (event.target.files[0]){
                    ...
                    if (file.size > 2000000) {
                        this.responseErrors = [...this.responseErrors, 'Le fichier ne doit pas dépasser 2 MO'];
                        if (this.user.data.avatar)
                            this.avatar = `${this.axios.defaults.baseURL}/storage/${this.user.data.avatar}`;
                        else
                            this.avatar = null;
                    ...
                    if (!supportedFormats.includes(file.type)){
                        this.responseErrors = [...this.responseErrors, 'Formats supporté: jpeg,png,jpg,gif'];
                        if (this.user.data.avatar)
                            this.avatar = `${this.axios.defaults.baseURL}/storage/${this.user.data.avatar}`;
                        else
                            this.avatar = null;
            ...
            saveAvatar() {
                this.$store.dispatch('uploadLoggedInUserAvatar', this.form.selectedFile).then( () => {
            ...
            deleteAvatar() {
                this.$store.dispatch('deleteLoggedInUserAvatar').then( () => {
                    this.responseSuccess = true;
                    this.form.selectedFile = null;
                    this.avatar = null;
                }).catch( err => {
        ...
        mounted() {
            this.user.data.avatar ? this.avatar = `${this.axios.defaults.baseURL}/storage/${this.user.data.avatar}` : null;
        },
        watch: {
            'user.data.avatar'(newVal) {
                newVal && (this.avatar = `${this.axios.defaults.baseURL}/storage/${newVal}`);
            },
            '$route.name'(newVal) {
                if (newVal === 'account') {
                    this.responseErrors = [];
...
<style scoped lang="scss">
    ...
    .site-header__saveAvatarBtn, .site-header__cancelAvatarBtn {
        display: block;
        margin-left: auto;
        margin-right: auto;
        padding: 5px 10px;
